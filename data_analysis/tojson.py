...
for num_P in range(len(PATH_list)): 
    ...
    row_tp = []
    with open('result/'+result_path+'/'+'position_teach_'+result_path+".csv", 'r') as f:
        ...
        with open("data/new_data/output-"+str(num_P+1)+".csv", 'rb') as f:
            result = chardet.detect(f.read())
            print(result['encoding'])
        with codecs.open("data/new_data/output-"+str(num_P+1)+".csv", 'r',encoding = 'gb18030' ) as f:
            # 创建CSV读取器
            reader = csv.reader(f)
        ...
        for c in range(1,4):
            ...
            for i_num,i in enumerate(x):
                if i['test_label']==c:
                    ...
                    #predict
                    predict = i['predict'][0].replace('\n', '').split('。')
                    for predict_e in predict:
                        if yes(["答案","回答"],predict_e) or predict_e=="是" or predict_e=="否" :

                            
                            if yes(wrong_str[c-1],predict_e) :
                                ap = 0
                            elif yes(unclear,predict_e) :
                                ap = 2       
                            elif yes(right_str[c-1],predict_e) :
                                ap = 1
                              
                            else:
                                ap = 2
                                # # print(i['predict'][0])
                                # pred_list = predict_e.split('，')
                                # if ap==2:
                                #     for j in pred_list:
            ...
            with open('result/'+result_path+'/teach_position_'+result_path+'.json','w') as f2:
                for data in x:
            ...
            try:
                ...
                with open('result/'+result_path+'/'+'teach_position_reslut.txt', way) as f:
                    f.write('relation:{}\n'.format(c))
                ...
                with open('result/'+result_path+'/'+'teach_position_reslut.txt', 'w') as f:
                    f.write('relation:{}\n'.format(c))
                    f.write("true_labels:{}\n,true:false={}:{}\n".format(true_labels,true_labels.count(1),true_labels.count(0)))
                    f.write("pred_labels:{}\n,true:false:unclear={}:{}:{}\n".format(pred_labels,pred_labels.count(1),pred_labels.count(0),pred_labels.count(2)))
                    f.write("unclear_labels:{}\n".format(num))
